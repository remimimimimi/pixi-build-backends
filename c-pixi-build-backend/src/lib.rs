...
use rattler_conda_types::{ParseStrictness, StringMatcher, Version, VersionSpec};
...
use rattler_conda_types::{MatchSpec, package::EntryPoint};
...
impl TryFrom<BinaryPackageSpecV1> for pixi_build_types::BinaryPackageSpecV1 {
    ...
    // TODO: Replace all unwraps with correct errors.
    fn try_from(value: BinaryPackageSpecV1) -> Result<Self, Self::Error> {
        Ok(Self {
            version: value.version.map(|s| {
                let string = s.to_str();
                VersionSpec::from_str(string, ParseStrictness::Lenient).unwrap()
            }),
            build: value.build.map(|s| {
                let string = s.to_str();
                StringMatcher::from_str(string).unwrap()
            }),
            build_number: value.build_number.map(|s| {
                let string = s.to_str();
                rattler_conda_types::BuildNumberSpec::from_str(string).unwrap()
            }),
            file_name: value.file_name.map(|n| n.into_string()),
            channel: value.channel.map(|u| Url::parse(u.to_str()).unwrap()),
            subdir: value.subdir.map(|s| s.into_string()),
            md5: value.md5.map(|md5| {
                let s = md5.to_str();
                // let digest = parse_digest_from_hex(s).unwrap();
                Md5Hash::clone_from_slice(s.as_bytes())
            }),
            sha256: value.sha256.map(|sha256| {
                let s = sha256.to_str();
                Sha256Hash::clone_from_slice(s.as_bytes())
            }),
            url: value.url.map(|u| Url::parse(u.to_str()).unwrap()),
            license: value.license.map(|l| l.into_string()),
        })
    }
}
...
// TODO: Error handling
#[ffi_export]
pub fn ppb_source_package_spec_v1_url_new(spec: UrlSpecV1) -> boxed::Box<SourcePackageSpecV1> {
    Box::new(SourcePackageSpecV1 {
        inner: pixi_build_types::SourcePackageSpecV1::Url(spec.try_into().unwrap()),
    })
    .into()
}
...
// TODO: Error handling
#[ffi_export]
pub fn ppb_source_package_spec_v1_git_new(spec: GitSpecV1) -> boxed::Box<SourcePackageSpecV1> {
    Box::new(SourcePackageSpecV1 {
        inner: pixi_build_types::SourcePackageSpecV1::Git(spec.try_into().unwrap()),
    })
    .into()
}
...
// TODO: Error handling
#[ffi_export]
pub fn ppb_package_spec_v1_binary_new(spec: BinaryPackageSpecV1) -> boxed::Box<PackageSpecV1> {
    Box::new(PackageSpecV1 {
        inner: pixi_build_types::PackageSpecV1::Binary(Box::new(spec.try_into().unwrap())),
    })
    .into()
}
...
// TODO: Error handling
#[ffi_export]
pub fn ppb_package_spec_v1_source_new(spec: BinaryPackageSpecV1) -> boxed::Box<PackageSpecV1> {
    Box::new(PackageSpecV1 {
        inner: pixi_build_types::PackageSpecV1::Binary(Box::new(spec.try_into().unwrap())),
    })
    .into()
    ...
    fn try_from(value: TargetSelectorV1) -> Result<Self, Self::Error> {
        Ok(match value.kind {
            ...
            TargetSelectorV1Kind::Platform => {
                // TODO: Error handling
                Self::Platform(value.platform.unwrap().into_string())
            }
        })
    }
}
...
impl TryFrom<TargetsV1> for pixi_build_types::TargetsV1 {
    ...
    fn try_from(value: TargetsV1) -> Result<Self, Self::Error> {
        Ok(Self {
            default_target: value.default_target.map(|t| (*t.into()).into()),
            targets: value.targets.map(|targets| {
                targets
                    .into_iter()
                    .map(|map| {
                        let map = map.clone();
                        let selector = map.selector.try_into().unwrap();
...
impl TryFrom<ProjectModelV1> for pixi_build_types::ProjectModelV1 {
    ...
    fn try_from(value: ProjectModelV1) -> Result<Self, Self::Error> {
        Ok(Self {
            name: value.name.map(|s| s.into_string()),
            version: value.version.map(|s| {
                let s = s.to_str();
                Version::from_str(s).unwrap()
            }),
            description: value.description.map(|s| s.into_string()),
            authors: value.authors.map(|authors| {
                authors
                    .into_iter()
                    .map(|author| author.clone().into_string())
                    .collect()
            }),
            license: value.license.map(|s| s.into_string()),
            license_file: value.license_file.map(|s| PathBuf::from(s.into_string())),
            readme: value.readme.map(|s| PathBuf::from(s.into_string())),
            homepage: value.homepage.map(|s| Url::parse(s.to_str()).unwrap()),
            repository: value.repository.map(|s| Url::parse(s.to_str()).unwrap()),
            documentation: value.documentation.map(|s| Url::parse(s.to_str()).unwrap()),
            targets: value
                .targets
                .map(|t| pixi_build_types::TargetsV1::try_from(*t.into()).unwrap()),
        })
    }
}
...
pub struct Generator {
    generator: Mutex<VirtualPtr<dyn CGenerator>>,
}
